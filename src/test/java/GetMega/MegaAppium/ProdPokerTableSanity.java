...
public class ProdPokerTableSanity extends ProdKobitonBase {
	...
	//@Test(enabled = true)
	public void pokerSitTable(AndroidDriver<WebElement> driver, ExtentReports extent) throws Exception {
	
		ExtentTest SitTable 	= extent.createTest("POKER : Sit-in Table");
		...
		try {
			...
			driver.findElementByAndroidUIAutomator("new UiScrollable(new UiSelector().scrollable(true).instance(0)).scrollIntoView(new UiSelector().textContains(\"Big Blind\").instance(0))").click();
			...
			pokerSwitchTable(driver, extent);
...
public void pokerSwitchTable(AndroidDriver<WebElement> driver, ExtentReports extent) {
		...
		//Initialise Logs and ham menu coordinates based on the current category
			...
			SwitchTable = extent.createTest("POKER : Switch Table");
			xCoordinates 					= 110;
			yCoordinates 					= 98;
		...
		try {
			...
			do {
				...
				if ((driver.findElements(By.id("com.mega.app:id/iv_menu")).size() > 0)) {
					driver.findElement(By.id("com.mega.app:id/iv_menu")).click();
			...
			//control passed to ProdLogout class
			ProdLogout pL = new ProdLogout();
			pL.Logout(driver, extent);
